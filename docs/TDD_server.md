...
- **`visible`** — server-resolved per-team boolean. Two flavors share this contract:
  ...
  - **Station view** (`atStation.tiles[].visible`) applies the **at-station privilege**: `faceUpOnMap(team, node) AND (mapVisibleSlot OR !locked)`. A slot whose claim-unlock timer has elapsed is station-visible even when the map-reveal timer has not — the Tier 2 spec from §3.3. The map view stays strict; only the team's own station relaxes.
...
4. **Split map view vs station view** in the JSON shape:
   ...
   - **`atStation`** — present when checked in; carries the same exhaustive `MapNodeTileDto[]` shape as `mapNodes[teamNode].tiles[]`. **Phase L Chunk 4 B-2 + L4 follow-up:** the original pre-L4 "see every station tile regardless of any timer" privilege is gone — claim-locked slots are still hidden at the station (Tier 3 before its claim window). A **narrower at-station privilege** survives, scoped to the *map-reveal* timer only: claim-unlocked-but-map-fogged slots (Tier 2 in `[0, 3600)` for the canonical preset) are exposed at the team's station even though the map view still hides them. `mapNodes[].tiles[].visible` always reflects the strict map rule; `atStation.tiles[].visible` may flip on earlier. `locked` is identical between the two surfaces.
...
- **Phase L Chunk 4 B-2 + at-station privilege:** `slot_map_unlock_offsets_seconds` gates `atStation.tiles[].visible` *softly* — when the slot's *claim* timer has elapsed, the slot is station-visible even before the map-reveal timer fires (Tier 2 from §3.3: `claim=0`, `map=3600` → station-visible at t=0, map-visible at t=3600). When neither timer has elapsed, `atStation.tiles[k]` matches `mapNodes[teamNode].tiles[k]` exactly: `tile: null, visible: false, locked: true`, and the client renders a "locked / hidden slot" countdown. The privilege is station-only — `mapNodes[teamNode].tiles[]` still uses the strict map rule (the team's own node looks fog-of-war'd to every *other* team's projection too). `locked` is identical between the two surfaces; only `visible` differs.
