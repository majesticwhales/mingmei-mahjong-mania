...
**Single source of truth (chunk 6):** Both rules above are exported as pure helpers in `server/src/services/slot-visibility.ts` (`isSlotUnlocked`, `assertSlotUnlocked`, `unlockedSlotIndices`, `slotMapUnlockAtMs`, `isSlotMapUnlocked`, `mapUnlockedSlotIndices`). The engine's `SWAP_TILE` handler and the projection layer both consume this module — the per-slot rules live in exactly one place.
...
Per-team progress on a single station challenge is tracked in `game_challenge_instances`. Three observable states (`status` + `cooldown_until`):
...
| Column                        | Meaning                                                                                                                                                                                                                          |
| ----------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `pending_swap_credit BOOLEAN` | True between `CHALLENGE_COMPLETED` and the next `SWAP_TILE` / `CLAIM_WIN`. Consumed on swap or claim. Resets on every `CHECK_IN` / `CHECK_OUT` (any unspent credit dies with the session — credits never bank across sessions).  |


Pacing between challenge attempts at the same station is enforced by the station-wide cooldown, **not** by a session-wide credit flag. Once the cooldown elapses a team can pick up the next challenge in the station's cycle without checking out and back in. The previous `credit_earned_in_session` flag (which forced a check-out cycle between completions) was removed in migration `20260613120000-drop-credit-earned-in-session.cjs`.
...
[cooldown (completed)]                                     │
   │                                                       │
   │ SWAP_TILE / CLAIM_WIN consumes the credit ──►         │
   │   pending_swap_credit = false                         │
   ▼                                                       ▼
(wait for cooldown_until)                            (wait for cooldown_until)
   │                                                       │
   ▼                                                       ▼
[available — picker advances to the next row in the cycle;
 team can START_CHALLENGE again without checking out]
```
...
- `UPDATE game_team_positions SET pending_swap_credit = FALSE WHERE game_team_id = <teamId>` — credit consumed.
...
2. Emit `GAME_ENDED` with payload `{ endedAt, endReason: "timer" | "all_teams_completed" | "manual", winningGameTeamId?: UUID }`. **`endReason` precedence:** if every team has `hand_completed_at`, the natural label `"all_teams_completed"` always wins, regardless of trigger; otherwise the value is `"manual"` when the admin "End game" path (`endGameEarly`) fired the transition and `"timer"` when the scheduler tick did. `winningGameTeamId` is set only when exactly one team has the strictly highest `final_points`; ties leave it `NULL` and the summary endpoint exposes the full ordering.
...
| `START_CHALLENGE`           | the calling team's channel                       | "🧩 Challenge: **{title}**. {description} *Document your proof in this channel.*" If the catalog row has a `discord_attachment_url` (e.g. a Sporcle link), it's appended.                                                                                      |
...
- `**visible`** — server-resolved per-team boolean. Two flavors share this contract:
  - **Map view** (`mapNodes[].tiles[].visible`): `faceUpOnMap(team, node) AND slot_map_unlock_offsets_seconds[slotIndex]` non-`NULL` AND `now >= started_at + offset * 1000`.
  - **Station view** (`atStation.tiles[].visible`) applies the **at-station privilege**: `true AND (mapVisibleSlot OR !locked)`. Two relaxations layered on top of the map rule: (a) `nodeFaceUp` is pinned to `true` because the team is at this node — §3.3 visit-based visibility says `faceUpForTeam(team, N) = true` regardless of phase fog or whether a `game_location_team_visibility` row exists yet; (b) a slot whose claim-unlock timer has elapsed is station-visible even when the map-reveal timer has not (the Tier 2 spec). The map view stays strict; only the team's own station relaxes.
  Both flavors collapse to `null` when the team has been hand-completed (`atStation` itself becomes `null` per §6.3; the map projection still shows tiles per the usual rules).
- `**locked`** — slot-lock layer mirror. True when `now < started_at + slot_unlock_offsets_seconds[slotIndex] * 1000`. Independent of `visible` — a slot can be visible-and-locked (StationPanel greys the "Swap" affordance), or locked-and-hidden (the map shows a placeholder shadow with a countdown). Identical between `mapNodes[].tiles[]` and `atStation.tiles[]` — only `visible` differs.
...
**Status:** shipped as **Phase L Chunk 4 B-3**. Route: `gamesRouter.get("/:id/nodes/:nodeId/view", ...)` in `[server/src/routes/games.ts](../server/src/routes/games.ts)`. Pre-Phase-L the client derived this from `atStation` + `mapNodes[]` + a pile of client-side rules; the endpoint moves all of it to the server.

```http
GET /api/games/:id/nodes/:nodeId/view
200 NodeViewDto
401 unauthorized                  # missing / invalid bearer token (requireAuth)
403 forbidden                     # requester is not a game participant — no enumeration: also returned for a non-existent gameId
404 game_not_found                # only reachable when the route layer skipped the participant gate (defence in depth)
404 node_not_found                # nodeId isn't on this game's map
409 game_not_started              # game row exists but `started_at` is null
409 game_ended                    # `games.status === "ended"` — switch to /summary
```

Authz: identical guard to `GET /:id/summary` — the user must own a `game_participants` row for the game; non-participants get `403 forbidden` regardless of whether the game exists (no enumeration channel). The team identifier passed to `buildNodeView` is the participant row's `game_team_id`, so the view is **always** scoped to the requesting user's team. No host override — even a host playing on team east sees team east's view.
...
- `**tiles[]**` — same exhaustive `MapNodeTileDto[]` shape as `mapNodes[].tiles[]` (§3.13), shared via the service helper described below. Per the at-station privilege, when the requesting team is checked in at this node, the `visible` flag relaxes to `nodeFaceUp && (mapVisible || !locked)` — claim-unlocked slots are exposed even before their map-reveal timer fires (matching the rule the projection's `atStation.tiles[]` follows). When the team is browsing some other node, the strict map rule applies. `locked` is always the same between the two flavors.
...
- `buildGameStateProjection` calls `buildNodeView` for the team's current station (the existing `atStation` payload becomes a thin wrapper around it — just adds the `pendingSwapCredit` flag, which is CHECK_IN-state rather than node-state).
...
### 3.15 Lobby creation presets

**Status:** live since the Phase L pre-work. Lobby creation in v1 ships **without a host config form** ([client TDD §5.3](TDD_client.md#53-lobbycontext) / [§7.3](TDD_client.md#73-wireframes)) — the only knob a host picks at create time is `**isTestGame: boolean`**, delivered as a checkbox on the Lobbies hub. The server picks every other knob from a pair of presets in `[server/src/game/lobby-presets.ts](../server/src/game/lobby-presets.ts)`.

```ts
// server/src/game/lobby-presets.ts
export const PRODUCTION_LOBBY_PRESET: LobbyGamePreset = {
  gameDurationSeconds: 14_400,           // 4h
  visibilityPhaseIntervalSeconds: 3_600, // 1h
  visibilityPhaseCount: 3,
  visibilityMode: "both",
  notifications: [
    // time-remaining warnings at 60 / 30 / 5 min before end
    { atSeconds: 10_800, template: "time_warning", data: { minutesLeft: 60 } },
    { atSeconds: 12_600, template: "time_warning", data: { minutesLeft: 30 } },
    { atSeconds: 14_100, template: "time_warning", data: { minutesLeft:  5 } },
  ],
};

export const TEST_LOBBY_PRESET: LobbyGamePreset = {
  gameDurationSeconds: 240,              // 4 min
  visibilityPhaseIntervalSeconds: 60,
  visibilityPhaseCount: 3,
  visibilityMode: "both",
  notifications: [
    { atSeconds: 180, template: "time_warning", data: { minutesLeft: 1 } },
    { atSeconds: 210, template: "time_warning", data: { minutesLeft: 0 } },
    { atSeconds: 235, template: "time_warning", data: { minutesLeft: 0 } },
  ],
};
...
| `challenge_cooldown_seconds`        | INT NOT NULL DEFAULT 300 CHECK `>= 0`. Per-(team, challenge) cooldown floor applied after a challenge resolves (completion or forfeit). Sourced from the chosen `LobbyGamePreset.challengeCooldownSeconds` (production: 300 / test: 5) on `createLobby`; host-editable via `updateConfig`; snapshotted to `games.challenge_cooldown_seconds` at start. See §3.8.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  |
...
| `slot_map_unlock_offsets_seconds`           | INTEGER[] NOT NULL DEFAULT `{0}`; snapshot from `lobby.slot_map_unlock_offsets_seconds` at start. Same length / per-element rules as on `lobbies` (slot 0 is `0`; `k >= 1` is `NULL` or `>= slot_unlock_offsets_seconds[k]`). Drives projection-time filtering of `mapNodes[].tiles[].visible` (Phase L §3.13): slot `k` is map-revealed once `now >= started_at + slot_map_unlock_offsets_seconds[k] * 1000`, or never if the entry is `NULL`.                           |
...
| `challenge_cooldown_seconds`                | INT NOT NULL DEFAULT 300 CHECK `>= 0`. Snapshot from `lobby.challenge_cooldown_seconds` at `GameStartService`. Read at resolution time via `challengeCooldownMsFromGame(ctx.game)` to stamp `game_challenge_instances.cooldown_until = resolved_at + challenge_cooldown_seconds * 1000`. See §3.8.                                                                                                                                                                          |
| `config_version`                            |                                                                                                                                                                                                                                                                                                                                                                                                                                                                           |
...
| Column                     | Notes                                                                                                                                                                                                         |
| -------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `pending_swap_credit`      | BOOLEAN NOT NULL DEFAULT FALSE. True between `CHALLENGE_COMPLETED` and the next `SWAP_TILE` / `CLAIM_WIN`. Consumed on swap / claim.                                                                          |


Resets to false on every `CHECK_IN` and `CHECK_OUT` (explicit or implicit). The previous `credit_earned_in_session` sticky-session flag was dropped in migration `20260613120000-drop-credit-earned-in-session.cjs` — pacing between challenge attempts is now solely enforced by the per-station cooldown on `game_challenge_instances.cooldown_until`. See §3.8.
...
4. **Split map view vs station view** in the JSON shape:
  ...
  - `**atStation`** — present when checked in; carries the same exhaustive `MapNodeTileDto[]` shape as `mapNodes[teamNode].tiles[]`. **Phase L Chunk 4 B-2 + L4 follow-up:** the original pre-L4 "see every station tile regardless of any timer" privilege is gone — claim-locked slots are still hidden at the station (Tier 3 before its claim window). A **narrower at-station privilege** survives, composed of two relaxations on top of the map rule: (1) **visit-based node reveal** — the team's currently-checked-in node is always face-up to them regardless of phase fog (§3.3 visit-based visibility), so the map view may render the node fogged for *every other team* + for the team's own *map* surface, but the team's `atStation.tiles[]` ignores the fog; (2) **claim-unlocked slot reveal** — claim-unlocked-but-map-fogged slots (Tier 2 in `[0, P)` for the canonical preset) are exposed even though the map view still hides them. `mapNodes[].tiles[].visible` always reflects the strict map rule; `atStation.tiles[].visible` may flip on earlier via either relaxation. `locked` is identical between the two surfaces.
...
| `atStation`                        | When checked in: `nodeId`, `code`, `tiles: MapNodeTileDto[]` (same per-slot shape as `mapNodes[teamNode].tiles[]`; the at-station privilege relaxes `tiles[].visible` for claim-unlocked-but-map-fogged slots — see §6.3), plus Phase H `currentChallenge` + `pendingSwapCredit`                          |
...
includedTiles =
  faceUpOnMap(team, node) ?
    [ { slotIndex: k, tile: ... }
      for k in mapUnlockedSlotIndices(game, game.slots_per_node, now)
      if a tile exists in placement (node, k) ]
    : []
```

- A slot whose `slot_map_unlock_offsets_seconds[k]` is `NULL` is **never** map-visible, regardless of phase. At the team's own station the at-station privilege kicks in: a `NULL` map timer + a non-`NULL` claim timer that has already elapsed makes the slot station-visible-but-map-hidden. If both the claim timer hasn't elapsed *and* the map timer is `NULL` (or hasn't elapsed), the slot stays hidden at the station too — `nodeFaceUp && (false || !locked)` collapses to false. The engine still accepts a `CLAIM_WIN` / `SWAP_TILE` targeting any slot if the team is willing to fire the command blindly (e.g. they learned the tile's `instanceId` from a previous projection where the slot was visible) — server-driven visibility gates the *render path*, not the command surface.
...
Both `mapUnlockedSlotIndices` and the claim-unlock helpers live in `server/src/services/slot-visibility.ts`; the projection layer must consume them directly rather than re-implementing the rules.
...
- **Phase L Chunk 4 B-2 + at-station privilege:** the at-station privilege is two cooperating relaxations of the map rule. (1) **Visit-based node reveal** — §3.3 visit-based visibility pins `nodeFaceUp` to `true` at the team's checked-in node, so phase fog (`game_location_team_visibility`) never hides the node from the team's own `atStation` surface (the map view still respects fog for other teams + for the team's view of other nodes). (2) **Claim-unlocked slot reveal** — `slot_map_unlock_offsets_seconds` gates `atStation.tiles[].visible` *softly*: a slot whose *claim* timer has elapsed is station-visible even before the map-reveal timer fires (Tier 2 from §3.3: `claim=0`, `map=3600` → station-visible at t=0, map-visible at t=3600). When neither relaxation flips a slot — node fogged AND map-fogged AND claim-locked — `atStation.tiles[k]` matches `mapNodes[teamNode].tiles[k]` exactly: `tile: null, visible: false, locked: true`. The privilege is station-only — `mapNodes[teamNode].tiles[]` still uses the strict map rule. `locked` is identical between the two surfaces; only `visible` can differ.
...
- **Mode gate:** When `games.visibility_mode` excludes the slot layer (`none` / `phase`), the slot-layer helpers short-circuit to "always unlocked / always map-visible" — both `mapNodes[].tiles[]` and `atStation.tiles[]` expose every slot index with `visible: true, locked: false`, ignoring the `slot_unlock_offsets_seconds` / `slot_map_unlock_offsets_seconds` snapshots (the at-station privilege has nothing to flip when `locked === false` everywhere). See §3.3.
...
- `currentChallenge`: the picked `game_node_challenges` row for the team's current station, joined to its catalog `challenges` row. `null` when the station has zero challenges configured (back-compat).
  ...
  - `status: "cooldown"` — the team's most recent resolved attempt at this node is still cooling down. `cooldownUntil` is an ISO 8601 timestamp; the client renders a countdown.
- `pendingSwapCredit`: boolean. True between `CHALLENGE_COMPLETED` and the consuming `SWAP_TILE` / `CLAIM_WIN`; the swap / claim button stays enabled while this is true. Reset on every `CHECK_IN` / `CHECK_OUT`.

Both fields are scoped to the issuing team — team A's `in_progress` is never visible to team B. The full per-state-machine projection contract lives in `server/src/projections/game-state.ts::buildCurrentChallenge`.
...
| `CHALLENGE_COMPLETED`       | player          | `gameTeamId`, `instanceId`, `nodeId`, `nodeCode`, `nodeName`, `challengeId`, `cooldownUntil`                                 |
...
| `GAME_ENDED`                | system          | `endedAt`, `endReason` (`timer` / `all_teams_completed` / `manual`), `winningGameTeamId?`                                    |
...
- `endReason: "all_teams_completed"` is new in Phase J; the legacy timer path emits `endReason: "timer"`. `endReason: "manual"` is emitted when the admin "End game" path (`endGameEarly`) fires the transition while at least one team is still incomplete.
- **Precedence:** `all_teams_completed` always wins when every team has `hand_completed_at` — the natural label beats the trigger. Otherwise the value is decided by where the transition originated: `manual` from `endGameEarly`, `timer` from the scheduler tick.
...
| **H** | **Honor-system swap gate — Shipped.** Three new commands + events (`START_CHALLENGE` / `CHALLENGE_COMPLETED` / `CHALLENGE_FORFEITED`) wire challenges to `SWAP_TILE` via a per-team swap credit. New tables `map_template_node_challenges` / `game_node_challenges` snapshot a per-station queue at game start (`bootstrapGameChallenges`). `game_challenge_instances` gains `game_node_challenge_id` + `cooldown_until` + extended `status` enum; `game_team_positions` gains `pending_swap_credit`. `CHECK_IN` / `CHECK_OUT` auto-forfeit any in-progress instance and reset the credit. `SWAP_TILE` rejects with `409 swap_credit_required` when the station has challenges and the team lacks a credit; back-compat path (no challenges configured) stays free-swap. `buildGameStateProjection` exposes `atStation.currentChallenge` + `pendingSwapCredit`. Pacing between challenge attempts at a station is enforced by `game_challenge_instances.cooldown_until` (station-wide); the originally-shipped `credit_earned_in_session` sticky-session flag was dropped in migration `20260613120000-drop-credit-earned-in-session.cjs` once the station-wide cooldown made it redundant. **Deferred follow-ups:** resolver-driven workflow (`ChallengeResolutionService` + `purpose = challenge_submission` media + reviewer queue) — the schema is migrated, just no v1 write path; the multi-challenge "discard the top card" mechanic for stations carrying `sort_order > 0` rows. See [§3.8](#38-challenges-honor-system-swap-gate).                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         |
...
- [x] Per-slot visibility rules schema, chunk 1 of the per-slot rollout (`20260530150000-add-per-slot-rules.cjs`): columns only, no CHECKs yet. Wrapped in a single transaction.
  - `map_templates`: add `default_slot_unlock_offsets_seconds INTEGER[] NOT NULL DEFAULT {0}`, `default_slot_map_visible BOOLEAN[] NOT NULL DEFAULT {true}` (the `slot_map_visible` boolean is later replaced by the nullable-element `slot_map_unlock_offsets_seconds` timeline in Phase L — see migration `20260611120000-add-slot-map-unlock-offsets.cjs`).
  - `lobbies`: add `slot_unlock_offsets_seconds`, `slot_map_visible` (same shape and defaults; same Phase L replacement).
...
- [x] Per-slot rules chunk 4 of the rollout (`20260530180000-add-slot-unlocked-job-type.cjs`): scheduler + engine lock validation. Wrapped in a single transaction.
  ...
  - Code: `game-start-service` now snapshots `lobby.slot_unlock_offsets_seconds` and `lobby.slot_map_visible` into `games` (the latter is replaced by `slot_map_unlock_offsets_seconds` in Phase L). `game-schedule-service.scheduleGameJobs` takes an options object (was positional) and seeds one `SLOT_UNLOCKED` job per slot `k >= 1` with a non-zero offset, at `startedAt + offset * 1000`, payload `{ slotIndex: k }`. Slot 0 (always 0) and any slot whose offset is 0 are skipped (already unlocked at start).
...
- [x] Phase H honor-system challenge wiring (`20260606000000-add-node-challenges.cjs`): adds the storage + per-team state needed for the challenge swap gate (§3.8). Wrapped in a single transaction.
  ...
  - `game_team_positions`: add `pending_swap_credit BOOLEAN NOT NULL DEFAULT FALSE`. Resets to false on every `CHECK_IN` / `CHECK_OUT` (explicit or implicit); `CHALLENGE_COMPLETED` flips it to true; `SWAP_TILE` / `CLAIM_WIN` clear it on success. The originally-paired `credit_earned_in_session` sticky-session column (also added here) was dropped in `20260613120000-drop-credit-earned-in-session.cjs` once the station-wide cooldown subsumed its role. See §3.8.
  - Engine code (chunks 2 + 3): new service `game-challenge-bootstrap.ts` (template → game queue copy at start); new handlers `start-challenge.ts` / `complete-challenge.ts` / `forfeit-challenge.ts` registered in `engine/handlers/index.ts`; shared `engine/challenge-lifecycle.ts` exporting `challengeCooldownMsFromGame` + `autoForfeitActiveChallenge` (used by `check-in.ts`, `check-out.ts`, and `claim-win.ts`; the cooldown is sourced from `games.challenge_cooldown_seconds`, not a global constant); `swap-tile.ts` extended with the credit gate (back-compat for zero-challenge stations); `recent-events.ts` lifts `challengeId` + `instanceId` to top-level on START / COMPLETED / FORFEITED events.
  - Projection (chunk 4): `buildGameStateProjection` exposes `atStation.currentChallenge` (`{ challengeId, title, description, flavorText, status, instanceId?, cooldownUntil? }`) plus `pendingSwapCredit`. New helper `buildCurrentChallenge` derives the three observable states (`available` / `in_progress` / `cooldown`) from the team's latest `game_challenge_instances` row.
...
- [x] Per-game challenge cooldown (`20260612120000-add-challenge-cooldown-seconds.cjs`): adds `lobbies.challenge_cooldown_seconds INTEGER NOT NULL DEFAULT 300` and `games.challenge_cooldown_seconds INTEGER NOT NULL DEFAULT 300` (both with a `>= 0` CHECK), replacing the legacy hard-coded `CHALLENGE_COOLDOWN_MS = 5 * 60 * 1000` constant in `engine/challenge-lifecycle.ts`. `LobbyGamePreset.challengeCooldownSeconds` carries the per-preset default (production: 300s / test: 5s); `createLobby` sources from the preset, the host can edit it via `updateConfig`, and `startFromLobby` snapshots `lobby.challenge_cooldown_seconds → games.challenge_cooldown_seconds`. The engine reads the per-game value via `challengeCooldownMsFromGame(ctx.game)` from `complete-challenge.ts`, `forfeit-challenge.ts`, and the shared `autoForfeitActiveChallenge` helper (called by `check-in.ts`, `check-out.ts`, `claim-win.ts`). Existing rows backfill to the legacy 5-minute floor via the column default. Wrapped in a single transaction; the `down` path drops both CHECK + column pairs.
...
- [x] Per-slot rules chunk 5 of the rollout (`20260530190000-add-config-array-checks.cjs`): lobby config flow + array-length / slot-0 invariants. Wrapped in a single transaction.
  ...
  - `lobby-service.createLobby`: `slotUnlockOffsetsSeconds` and `slotMapVisible` default to the map template's `defaultSlotUnlockOffsetsSeconds` / `defaultSlotMapVisible`; host can override either independently; explicit values must match `slotsPerNode` exactly (no silent resize on create). The `slotMapVisible` boolean field is replaced by `slotMapUnlockOffsetsSeconds` in Phase L (`20260611120000-add-slot-map-unlock-offsets.cjs`); the resize / override / default semantics carry over to the new field with `0` substituted for `true` (visible immediately) and `NULL` substituted for `false` (never on map).
