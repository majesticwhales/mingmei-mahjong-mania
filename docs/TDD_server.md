...
| `atStation`                        | When checked in: `nodeId`, `code`, `tiles: MapNodeTileDto[]` (same per-slot shape as `mapNodes[teamNode].tiles[]`; the at-station privilege relaxes `tiles[].visible` for claim-unlocked-but-map-fogged slots — see §6.3), plus Phase H `currentChallenge` + `pendingSwapCredit` + `creditEarnedInSession` |
