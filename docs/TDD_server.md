...
- **`visible`** — server-resolved per-team boolean. True iff `faceUpOnMap(team, node)` is true AND the slot's map-reveal timer has elapsed (`slot_map_unlock_offsets_seconds[slotIndex]` is non-`NULL` and `now >= started_at + offset * 1000`), *and* the team has not been hand-completed (a hand-completed team's `atStation` collapses to `null` per §6.3; the map projection still shows tiles per the usual rules).
