...
- `<StationPanel />` *(shipped Phase L Chunk 5)* — takes `viewingNodeId: string | null` and sources tiles + currentChallenge + availableActions from [`useNodeView(viewingNodeId)`](../client/src/state/game/useNodeView.ts). `viewingNodeId` is whatever the user is *viewing* — the team's current station by default, or whichever node they tapped on the map. The projection's `atStation` (read via `useAtStation()` inside the panel) is still consulted but only for "where am I checked in right now" — it picks between the "At …" / "Checked in at …" eyebrow variants and gates the at-station action cluster (Swap / Claim / Check out) behind `isViewingCheckedInStation`. The action buttons read their `enabled` + `reason` directly from `NodeViewDto.availableActions[]`; a `reasonCopy()` table inside the component maps each reason code to tooltip text. The swap / challenge button toggle is **purely server-driven** off `availableActions[].swap_tile.reason === "swap_credit_required"` — the Phase H interim `showChallengeOverride` prop and the `lib/challengeContext` scaffold helpers (`buildScaffoldChallenge`, `isScaffoldChallenge`, `needsChallengeBeforeSwap`) were retired once every tile station got a real `currentChallenge` from `server/seeders/data/challenges/ttc-2026.json` (see TDD_server §3.8 *Content authoring*). The lone helper kept in `lib/challengeContext.ts` is `resolveCheckedInChallenge(atStation)` — a one-liner over `atStation?.currentChallenge ?? null` that GameScreen still calls so the call-site documents intent.caffold-only stations — `GameScreen` still derives that flag from `lib/challengeContext`.
...
- `client/src/screens/Game/{SwapTileModal,SwapLocationTilesModal,ClaimWinModal,…}.tsx` and the `GameScreen` direct call sites (`handleCheckIn`, `handleCheckOut`, `handleCompleteChallenge`, `handleAbandonChallenge`) — every command site swaps its `useGame().checkIn(...)` / `submitCommand(...)` call for `useCommandWithGeo(...)`. The existing `useGeolocation` hook becomes an internal primitive of `useCommandWithGeo` only.
